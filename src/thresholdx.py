...
fg.logo(
    "https://assets-global.website-files.com/5b27a494dc3d3103abc01d83/5f2387c90e9e1dca5453fc23_web1.png"
)
...
for post in posts:
    ...
    title = assert_some(post.find(class_="t-invert")).text
    sim_tags = assert_some(post.find(class_="flexh")).find_all("div")
    sim_tag = [tag for tag in sim_tags if "w-condition-invisible" not in tag["class"]][
        0
    ].text

    image = post.find("img")
    url = assert_type(assert_some(post.find("a"))["href"], str)
    vsp = assert_some(post.find(class_="vsp")).text
    date = datetime.strptime(vsp, "%b %d, %Y").astimezone(pytz.UTC)
    ...
    if body_response.ok:
        ...
        paragraphs = assert_some(soup.find(class_="w-richtext")).find_all("p")
